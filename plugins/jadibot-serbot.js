...
let handler = async (m, { conn, args, usedPrefix, command, isModeration, text }) => {
  ...
  let id = `${who ? who.split(`@`)[0] : 'unknown'}`  
...
export async function sanJadiBot(options, text) {
  ...
  try {
    ...
    if (m && typeof m.reply === 'function') m.reply(`❀ *Use correctamente el comando »* ${usedPrefix + command} code`)
    return
  }
