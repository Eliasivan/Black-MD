async function handler(m, { conn, args, usedPrefix, command }) {
  ...
  if (!args[0] || !args[1]) {
    const helpMessage = `${emoji} Debes mencionar a quien quieras regalar *${money}*.\n> Ejemplo » *${usedPrefix + command} 25000 @mencion*`.trim();
  ...
  if (user[bankType] * 1 < count) return conn.sendMessage(m.chat, {text: `${emoji2} No tienes suficientes ${money} en el banco para transferir.`, mentions: [m.sender]}, {quoted: m});
