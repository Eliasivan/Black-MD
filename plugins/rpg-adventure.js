...
let handler = async (m, { conn }) => {
    ...
    if (user.lastAdventure && new Date() - user.lastAdventure <= 1500000) {
        ...
        return conn.reply(m.chat, `⏳ Debes esperar ${msToTime(timeLeft)} antes de aventurarte de nuevo.`, m);
    }
    ...
    let stars = pickRandom([20, 5, 7, 8, 88, 40, 50, 70, 90, 999, 300]); // Estrellas en lugar de monedas
    ...
    user.stars = (user.stars || 0) + stars; // Cambiado de "coin" a "stars"
